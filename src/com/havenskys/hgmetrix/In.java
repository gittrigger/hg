package com.havenskys.hgmetrix;
...
public class In extends Activity implements InRenderer.AnimationCallback {
	...
	private static String TAG = "In";private static String G = "In";
    ...
    private World mkGLwork() {
        ...
        world.mTrimy = 0f;
        world.mTrimz = 0f;
        world.mPivotx = 100.4f;//
        world.mPivoty = 1.8f;//y tall
        world.mPivotz = 0.0f;
 
        int t = 1;int j;
    	for(t = 0;t<mPurpose.length;t++){
    	    if(mPurpose[t] != null){}else{
    	    	mPurpose[t] = new InPurpose(InPurpose.kAxisX);
    	    	mPart[t][0]  = new InPartShape(world, -10.7f, -10.2f, -13.7f, 14.5f, 16.7f, 14.5f);
    	    	for(j=0;j<6;j++){if(j==0||j==5){mPart[t][0].setFaceColor(j,black);continue;}mPart[t][0].setFaceColor(j,orange);}
                world.addShape(mPart[t][0]);



t++;
mPurpose[t] = new InPurpose(InPurpose.kAxisY);
...
// orbit 
    private World mkConsole() {
        ...
        int t = 1;
    	for(t = 0;t<mPurpose.length;t++){
    	    if(mPurpose[t] != null){continue;}else{
console = t;
mPurpose[t] = new InPurpose(InPurpose.kAxisY);
mPurpose[t].setAngle(0f);
//mPurposeU[t] = 0f;
//mPurposeU[t] = 10f;
//mPart[t][0] = new InPartShape(world,0,90,108f,52f,125,110f);
//mPart[t][1] = new InPartShape(world,(0f),(60),(108f), (52f), (85.0f), (110f) );
//mPart[t][2] = new InPartShape(world,(52f),(60),(108f), (140f), (62.0f), (110f) );
//mPart[t][3] = new InPartShape(world,(52f),(54),(108f), (140f), (56.0f), (110f) );
mPart[t][0] = new InPartShape(world,(0f),(0),(108f), (52f), (50.0f), (110f) );
//mPart[t][5] = new InPartShape(world,(0f),(-50f),(108f), (52f), (25.0f), (110f) );
int j = 1;for(int i = 0; i <= 0 && mPart[t] != null; i++){for(j=0;j<6;j++){if(j==0||j==5){mPart[t][i].setFaceColor(j,black);continue;}mPart[t][i].setFaceColor(j,orange);}}
world.addShape(mPart[t][0]);
...
// console    
    ...
    int lathing = 0;int orbitlimit = 1;int console = 1;
    private World mkWorld() {
        ...
        //mPart[0][0]  = new InPartShape(world, 3.5f, 1.76f, 2.0f, 3.76f, 2.0f, 2.76f);
/*        mPart[0][1]  = new InPartShape(world, 3.0f, 4.0f, 3.0f, 3.3f, 5.0f, 4.0f);
        mPart[0][2]  = new InPartShape(world, 3.0f, 4.1f, 4.5f, 3.3f, 5.1f, 6.5f);
        mPart[0][3]  = new InPartShape(world, 3.0f, 4.2f, 7.0f, 3.3f, 5.2f, 8.0f);
        mPart[0][4]  = new InPartShape(world, 3.0f, 2.5f, 3.0f, 3.3f, 3.5f, 4.0f);
        mPart[0][5]  = new InPartShape(world, 3.0f, 2.6f, 4.5f, 3.3f, 3.6f, 6.5f);
        mPart[0][6]  = new InPartShape(world, 3.0f, 2.7f, 7.0f, 3.3f, 3.7f, 8.0f);
//        mPart[0][0]  = new InPartShape(world, -5.0f, -1.5f, -1.0f, 5.5f, -0.2f, 1.1f);

        mPart[4][0]  = new InPartShape(world, 3.5f, 2.0f, -2.0f, 3.76f, 3.0f, -0.76f);        
        mPart[4][1]  = new InPartShape(world, 3.0f, 4.0f, 3.0f, 3.3f, 5.0f, 4.0f);
        mPart[4][2]  = new InPartShape(world, 3.0f, 4.1f, 4.5f, 3.3f, 5.1f, 6.5f);
        mPart[4][3]  = new InPartShape(world, 3.0f, 4.2f, 7.0f, 3.3f, 5.2f, 8.0f);
        mPart[4][4]  = new InPartShape(world, 3.0f, 2.5f, 3.0f, 3.3f, 3.5f, 4.0f);
        mPart[4][5]  = new InPartShape(world, 3.0f, 2.6f, 4.5f, 3.3f, 3.6f, 6.5f);
        mPart[4][6]  = new InPartShape(world, 3.0f, 2.7f, 7.0f, 3.3f, 3.7f, 8.0f);
//        mPart[4][0]  = new InPartShape(world, 3.5f, 2.0f, -2.0f, 3.76f, 3.0f, -0.76f);        
//*/
        //mPart[1][0]  = new InPartShape(world, -11.0f, 0.32f, -1.0f, -10.32f, 1.0f, -0.32f);
        //mPart[0][0]  = new InPartShape(world, 2.5f, 3.5f, 9.5f, 3.5f, 6.5f, 0.5f);        
        //mPart[0][1]  = new InPartShape(world, 3.5f, 3.76f, 9.5f, 4.5f, 2.0f, 2.0f);
//        mPart[0][0]  = new InPartShape(world, -0.7f, 3.5f, 9.5f, 0.7f, 6.5f, 0.5f); 
//        mPart[1][0]  = new InPartShape(world, -0.7f, 3.5f, 9.5f, 0.7f, 6.5f, 0.5f); 
...
//        mPart[3][0]  = new InPartShape(world, -5.0f, -4.0f, -5.0f, 5.0f, -3.5f, 5.0f);

        //mXthing = new InPurpose(InPurpose.kAxisX);
        //mBthing = new InPurpose(InPurpose.kAxisY);
        mPurpose[0] = new InPurpose(InPurpose.kAxisY);//spin horizontally
        mPurpose[1] = new InPurpose(InPurpose.kAxisZ);    
        mPurpose[2] = new InPurpose(InPurpose.kAxisX);
        mPurpose[3] = new InPurpose(InPurpose.kAxisX);
        //mPurpose[0].mAxis = InPurpose.kAxisX;mPurpose[0].setAngle(155);mPurpose[0].mAxis = InPurpose.kAxisY;
        //mPurpose[1].mAxis = InPurpose.kAxisY;mPurpose[1].setAngle(155);mPurpose[1].mAxis = InPurpose.kAxisZ;
        //mPurpose[2].mAxis = InPurpose.kAxisZ;mPurpose[2].setAngle(155);mPurpose[2].mAxis = InPurpose.kAxisX;
       ...
       t++;mPart[t][0] = new InPartShape(world,-1.5f,4f+(g*0.15f)+(g*0.12f),7.8f+(g*g*0.015f),1.5f,8f+(g*0.15f)+(g*0.12f),8f+(g*g*0.015f));
       }
       t = lathing;
       for(int g=0;g<GRAPH_SIZE;g++){
	   t++;mPurpose[t] = new InPurpose(InPurpose.kAxisY);
	   mPurposeU[t] = g*g* 0.25f;
	   mPurpose[t].setAngle(mPurposeU[t]);
       }
 
       for(int g=0;g<GRAPH_SIZE;g++){//left bottom back -- right top front
           t++;
        	   mPart[t][0] = new InPartShape(world,-1.5f,4f-(g*0.15f)-(g*0.12f),9.8f+(g*g*0.015f),1.5f,8f-(g*0.15f)-(g*0.12f),10f+(g*g*0.015f));
        	   mPurposeU[t] = g*g* 0.25f;
        	   mPurpose[t] = new InPurpose(InPurpose.kAxisY);
        	   mPurpose[t].setAngle(mPurposeU[t]);
       }
       ...
       for(int g=0;g<GRAPH_SIZE;g++){//left bottom back -- right top front
       t++;
    	   mPart[t][0] = new InPartShape(world,-1f,-3f+(g*0.5f),22.8f-(g*0.2f),5f,2f+(g*0.5f),23f-(g*0.2f));
    	   mPurposeU[t] = g*g*0.25f;
    	   mPurpose[t] = new InPurpose(InPurpose.kAxisY);
           mPurpose[t].setAngle(mPurposeU[t]);
           //world.loadTexture((GL10) mPurpose[t],mCtx,R.drawable.redt);
       }
...
//*/
        ...
        //        	mPurpose[t].setAngle(f*0.15f);
        	//mPart[t][0] = new InPartShape(world,-1f,4f+g*0.1f,8.8f+g*0.1f,1f,7f+g*0.1f,9f+g*0.1f);
...
//    for(j=0;j<6;j++){if(j==0||j==5){mPart[2][0].setFaceColor(j,white);continue;}mPart[2][0].setFaceColor(j,blue);}
      //for(t = 0;t<4;t++){for(j=0;j<6;j++){if(j==0||j==5){mPart[t][0].setFaceColor(j,black);continue;}mPart[t][0].setFaceColor(j,orange);}}
...
float mRAngleY = 0f; float mRAngleX = -180; float mRAngleZ = 0;
float mRecentY = 10f; float mRecentX = 0f; float mRecentZ = 0f;
float m1Ay = 0f; float m1Ax = 360f; float m1Az = 0;
float m1Cy = 0f; float m1Cx = -10f; float m1Cz = -140f;
boolean mAnPause = false;boolean mRelease = false;
OnTouchListener listtouch = new OnTouchListener(){public boolean onTouch(View v, MotionEvent ev) {
...
if(mAnPause){mRelease = true;}else{mAnPause = true;record = true;Log.w(G,"GL View angle(y"+mRenderer.mAngleY+",x"+mRenderer.mAngleX+",z"+mRenderer.mAngleZ+") camera("+mRenderer.mLCameraY+","+mRenderer.mLCameraX+","+mRenderer.mLCameraZ+")");
    }}

else if(act == MotionEvent.ACTION_UP){reani.sendEmptyMessageDelayed(1,2000);if(mRelease){mAnPause = false;mRelease = false;record = true;Log.w(G,"GL View angle(y"+mRenderer.mAngleY+",x"+mRenderer.mAngleX+",z"+mRenderer.mAngleZ+") camera("+mRenderer.mLCameraY+","+mRenderer.mLCameraX+","+mRenderer.mLCameraZ+")");
    }}
	...
	if(record){		
        ...
        Uri source = SqliteWrapper.insert(mCtx, getContentResolver(), Uri.withAppendedPath(DataProvider.CONTENT_URI, "perspective"), ins);
        Log.i(TAG,"Created " + source.toString());
	}

/*
   if( act == MotionEvent.ACTION_MOVE){
        float dx = mPreviousX - x; float ox = x>mPreviousX?x - mPreviousX:mPreviousX - x;if(ox*-1 > 0){ox *= -1;}
        float dy = mPreviousY - y; float oy = y>mPreviousY?y - mPreviousY:mPreviousY - y;if(oy*-1 > 0){ox *= -1;}            

if( ox > 2 || oy > 2){mLMove = SystemClock.uptimeMillis();}

//if( !mHome && mAtHome ){return true;}
//if(mHome && (ox > 5f || oy > 5f) ) { mHome = false; mAtHome = false; }else if(mHome){return true;}

//if( mRelease && (ox > 5 || oy > 5) ){mRelease = false;}else{return true;}

        if( ox > 2 && oy > 2 ){mRenderer.mLCameraZ += dx * (TOUCH_SCALE_FACTOR * -0.22) + dy * (TOUCH_SCALE_FACTOR * -0.22);}
        else if(oy > ox){
float zd = (mRenderer.mLCameraZ <= -1?(-1 * mRenderer.mLCameraZ):mRenderer.mLCameraZ);
        mRenderer.mCameraX = mRenderer.mLCameraX + dy * TOUCH_SCALE_FACTOR * zd == 0?1:zd;
        if(mRenderer.mLCameraX > 80){mRenderer.mCameraX = 80;}        
if(mRenderer.mLCameraX < -120){mRenderer.mCameraX = -120;}

}else if(ox > oy ){

mRenderer.mAngleX += dx * TOUCH_SCALE_FACTOR * -1;

}else{

Log.w(G,"no dx("+dx+") "+mPreviousX+"->"+x+" \t\t - dy("+dy+") "+mPreviousY+"->"+y+"");
...
//       mRenderer.mAngleX = (float) ((int) mRenderer.mAngleX);       mRenderer.mCameraY = (float) ((int) mRenderer.mCameraY);
//       mRenderer.mCameraZ = (float) ((int) mRenderer.mCameraZ);       mRenderer.mCameraX = (float) ((int) mRenderer.mCameraX);

//Log.w(G,"dx("+dx+") "+mPreviousX+"->"+x+" \t\t - dy("+dy+") "+mPreviousY+"->"+y+"");
//        if(mRenderer.mLCameraZ < -120f || mRenderer.mLCameraZ > 120f || mAtHome ){mHome = false; mAtHome = true;mRenderer.mCameraZ = m1Cz;mRenderer.mCameraX = m1Cx;mRenderer.mCameraY = m1Cy;mRenderer.mAngleX = m1Ax;mRenderer.mAngleY = m1Ay;mRenderer.mAngleZ = m1Az;}
//        if ( (mRenderer.mLAngleX > 65  && mRenderer.mLAngleX < 115) || (mRenderer.mLAngleX < 25 && mRenderer.mLAngleX > 335) ){       }
//        mRenderer.mCameraZ = mRenderer.mCameraZ + dy * TOUCH_SCALE_FACTOR * 1.12f * (mRenderer.mLCameraZ <= -1?(-1 * mRenderer.mLCameraZ):mRenderer.mLCameraZ);
//mRenderer.mAngleY += dy * TOUCH_SCALE_FACTOR * -1 * (mPreviousX<x?1:-1);
//        mRenderer.mCameraX = mRenderer.mCameraX + dx * TOUCH_SCALE_FACTOR * (1 + ( -0.02f * (mRenderer.mLCameraZ <= -1?(-1 * mRenderer.mLCameraZ):mRenderer.mLCameraZ)));
//        Log.w(G,"touch " + mRenderer.mLAngleX + "("+dx+") " + mRenderer.mLAngleY + "("+dy+")");
        //requestRender();
...
private boolean mAtHome = true;
    ...
    private final int GRAPH_SIZE = 60;
...
mRenderer.mCameraY = mRecentY;
mRenderer.mCameraX = mRecentX;
mRenderer.mCameraZ = mRecentZ;
mRenderer.mAngleY = mRAngleY;
mRenderer.mAngleX = mRAngleX;
mRenderer.mAngleZ = mRAngleZ;
...
mGLSurfaceView.setOnTouchListener(listtouch);
        ...
        //mText.append("\n\nStart>");
        /*
{Message msg = new Message();Bundle b = new Bundle();b.putString("message", "Mode " + mode);msg.setData(b);mToastHandler.sendMessage(msg);}
       //*/ 
...
{SensorService.sendEmptyMessageDelayed(2,pRate);}
	...
	Thread t = new Thread() {
            public void run() {
            	...
            	//SyncProcessing syncProcess = new SyncProcessing(getApplicationContext());
...
mAtHome = false;
        ...
        mRenderer.mAngleX += e.getX() * TRACKBALL_SCALE_FACTOR *  1.2f;// * (mRenderer.mLCameraZ <= -1?(-1 * mRenderer.mLCameraZ):mRenderer.mLCameraZ);
        //Log.w(G,"trackball " + mRenderer.mLCameraX + "("+(e.getX() * TRACKBALL_SCALE_FACTOR)+") " + mRenderer.mLCameraY + "("+(e.getY() * TRACKBALL_SCALE_FACTOR)+")");
//        mRenderer.mAngleY += e.getY() * TRACKBALL_SCALE_FACTOR * -1.2f;// * (mRenderer.mLCameraZ <= -1?(-1 * mRenderer.mLCameraZ):mRenderer.mLCameraZ);
        //requestRender();
...
//                                                    valence2 = (lastvalues[b]>values[b]?lastvalues[b]-values[b]:values[b]-lastvalues[b]);}
//                                                	float valence3 = 1f;{b = 1;
//                                                    valence3 = (lastvalues[b]>values[b]?lastvalues[b]-values[b]:values[b]-lastvalues[b]);}
...
// orbit
for(int t = lathing+1; t < mPurpose.length && t < orbitlimit && mPurpose[t] != null && !mAnPause; t++){mPurposeU[t] = values[b]*(t*t*-0.035f);}

mPurposeU[1] = (values[b]) * TOUCH_SCALE_FACTOR; //rotate


//mPurpose[1].setAngle(values[b] * TOUCH_SCALE_FACTOR);
//mPurpose[3].setEt ( mPurpose[3].x+values[b]*15*TOUCH_SCALE_FACTOR , mPurpose[3].y, mPurpose[3].z );
//camera y(up down) x(left-right) z(in-out)
//    mRenderer.mAngleX = values[b] * TOUCH_SCALE_FACTOR;//mRenderer.mLAngleX-((mRenderer.mAngleX < values[b]?values[b]-mRenderer.mAngleX:mRenderer.mAngleX-values[b])*0.25f);
//mRenderer.mCameraX = values[b]*(5*5*-0.035f);	
//mWorlds[1].mPivotx += 
//mPurposeU[3] = values[b] * TOUCH_SCALE_FACTOR;	
...
//        mPurpose[0].setAngle(values[b] * TOUCH_SCALE_FACTOR);

//	mCurrentAngle = values[b]*0.15f;
//	mAthing.setAngle(mCurrentAngle);
	//for(int w=0; w < mWorlds.length && mWorlds[w] != null; w++){
	//	}
//    mRenderer.mAngleY = valence * 36; //(values[b]*26.5f)+mRAngleY;
//    mRenderer.mAngleY = values[b]*TOUCH_SCALE_FACTOR;// mRenderer.mLAngleY-((mRenderer.mAngleY < values[b]?values[b]-mRenderer.mAngleY:mRenderer.mAngleY-values[b])*0.25f);
...
//        mPurpose[2].setAngle(values[b] * TOUCH_SCALE_FACTOR);


	if(mAnPause){
        //mRenderer.mAngleX = (values[b]*-1.5f)+mRAngleX;
	}
	
    //mRenderer.mAngleX = mRAngleX-(values[b]*-4.5f)-46;
    //mRenderer.mAngleY = mRAngleY+(values[b]*-4.5f);
...
if(!mHome && mAtHome && mRenderer.mLCameraZ == m1Cz && mRenderer.mLCameraX == m1Cx && mRenderer.mLCameraY == m1Cy && mRenderer.mAngleX == m1Ax && mRenderer.mAngleY == m1Ay && mRenderer.mAngleZ == m1Az ){
{Message msg = new Message();Bundle b = new Bundle();b.putString("message", "Home");msg.setData(b);mToastHandler.sendMessage(msg);}
mHome = true;
}

if((mRenderer.mLCameraZ < -250f || mRenderer.mLCameraZ > 250f || mAtHome  ) && !(mRenderer.mLCameraZ == m1Cz && mRenderer.mLCameraX == m1Cx && mRenderer.mLCameraY == m1Cy) ){mAtHome = true;mRenderer.mCameraZ = m1Cz;mRenderer.mCameraX = m1Cx;mRenderer.mCameraY = m1Cy;mRenderer.mAngleX = m1Ax;mRenderer.mAngleY = m1Ay;mRenderer.mAngleZ = m1Az;}
// orbit



	int t, j, i, k;
	float incm = 0; float it = 0; float od = 0f;
	    for(t=0;t<mPurpose.length;t++){
	        if(mPurpose[t]!=null){
...
//mPurpose[console].setEt(mRenderer.mLCameraX-1f, mRenderer.mLCameraY-1f, (mRenderer.mLCameraZ-1f));
//mPurposeU[console] = (mRenderer.mAngleY-180);
//mPurpose[console].setAngle(mRenderer.mAngleX-180);
}

	        if(mAnPause && t > lathing){mPurposeU[t] = mPurpose[t].mAngle;continue;}else{
        	if(mPurposeU[t] < 0 && mPurposeU[t] > -1){mPurposeU[t] = 0;}
        	if( mPurposeU[t] != mPurpose[t].mAngle){
	        	
                        // normalize float size to two places
	        	if(!mPurpose[t].mAnimating){mPurpose[t].startAnimation(); int h4 = (int) (mPurposeU[t] * 100f); mPurposeU[t] = (float) h4 * 0.01f;}
	    	   
                     
	    	    od = (mPurposeU[t] > mPurpose[t].mAngle?(mPurposeU[t] - mPurpose[t].mAngle):(mPurpose[t].mAngle - mPurposeU[t]));//(mPurpose[t].mAngle *(float) Math.PI/2f);
	    	    incm = od * 0.01f; 
                    if(incm > 0.01f){
	    	    incm = 0.05f;
                    if(od < 0.05f){incm = 0.05f;}
	    	    if(od >= 100f){incm *= 2.01f;}
	    	    else if(od < 10f){incm *= 0.14f;}
	    	    //else if(od < 100f){incm *= 1.2f;}
	    	    //else if(od < 500f){incm *= 0.08f;}

//if(t > orbitlimit){
//Log.w(G,"od("+od+") incm("+incm+")");
//}
            	...
            	if(mPurpose[t].mAngle == mPurposeU[t]){
                    ...
                    if(it > mPurposeU[t]){it = mPurposeU[t];}
            	}else{
            		it = mPurpose[t].mAngle-incm; 
                    //if(mPurposeU[t] < 0.02f){mPurposeU[t] = 0.02f;}
            		if(it < mPurposeU[t]){it = mPurposeU[t];}
            	}
       	...
       	mPurpose[t].setAngle(mPurpose[t].mAngle);//mPurpose[t].mAngle);
	        mPurpose[t].endAnimation();
//mPurpose[t].mAnimating = false;	        	
                }else{
if(t > orbitlimit){
                	Log.w(G,"animate() "+t+" od("+od+")" + incm + "+" + mPurpose[t].mAngle + " in "+it+" to " + mPurposeU[t]);
}
                	mPurpose[t].setAngle(it);
                }
        InPart[] ground;for(i = 0, k=0; i < mPart[t].length && mPart[t][i] != null;i++) {ground = mPurpose[t].mShapes;ground[k++] = mPart[t][i];}
	    	    continue;
	    	    }
	    	}}
//        	Log.w(G,"animate() "+t+" " + mPurpose[t].mAngle + " in " + mPurposeU[t]);
...
int mSelected = -1;
    public boolean onMenuItemSelected(int featureId, MenuItem item) {
        ...
        case 1:
{Message ml= new Message(); Bundle bl = new Bundle(); bl.putString("message","Make"); ml.setData(bl); mToastHandler.sendMessage(ml);}
            return true;
        case 2:         
{Message ml= new Message(); Bundle bl = new Bundle(); bl.putString("message","Things"); ml.setData(bl); mToastHandler.sendMessage(ml);}
            return true;
        case 3:         
{Message ml= new Message(); Bundle bl = new Bundle(); bl.putString("message","Now"); ml.setData(bl); mToastHandler.sendMessage(ml);}
...
{Message ml= new Message(); Bundle bl = new Bundle(); bl.putString("message",""+t+" ("+mx+","+my+","+mz+") ("+tx+","+ty+","+tz+")"); ml.setData(bl); mToastHandler.sendMessage(ml);}


setCamera(my,mz,mx-50,pRate * 8);
...
{Message ml = new Message(); Bundle bl = new Bundle(); bl.putFloat("x",x); bl.putFloat("y",y); bl.putFloat("z",z); ml.setData(bl); mCamera.sendMessageDelayed(ml,msec);}
...
{cx.add(4,3000,0,"Camera X " + mRenderer.mCameraX);}// s.add(3000,3100,0,((int)(mRenderer.mCameraX)+10)+"");s.add(3000,3100,1,((int)(mRenderer.mCameraX)-10)+"");s.add(3000,3100,0,""+0);s.add(3000,3100,2,m1Cx+"");}
{cx.add(4,3001,0,"Camera Y " + mRenderer.mCameraY);}// s.add(3000,3101,0,((int)(mRenderer.mCameraY)+10)+"");s.add(3000,3101,1,((int)(mRenderer.mCameraY)-10)+"");s.add(3000,3101,0,""+0);s.add(3000,3101,2,m1Cy+"");}
{cx.add(4,3002,0,"Camera Z " + mRenderer.mCameraZ);}// s.add(3000,3102,0,((int)(mRenderer.mCameraZ)+10)+"");s.add(3000,3102,1,((int)(mRenderer.mCameraZ)-10)+"");s.add(3000,3102,0,""+0);s.add(3000,3102,2,m1Cz+"");}
...
{
SubMenu mm = menu.addSubMenu(0, 1, 1, "Make");
...
//			msg.setData(b2);
//			mToastHandler.sendMessage(msg);}
			//mDataStore.updateEntry("syncContent", "status", 0, "contentType=\"owa_calendar\"");
	        return true;
		case 3:
{Message ml= new Message(); Bundle bl = new Bundle(); bl.putString("message","Now"); ml.setData(bl); mToastHandler.sendMessage(ml);}
	        {Intent i = new Intent(mCtx, com.havenskys.galaxy.Motion.class);
...
{Message ml= new Message(); Bundle bl = new Bundle(); bl.putString("message",""+item.getTitle()); ml.setData(bl); mToastHandler.sendMessage(ml);}
return true;
}
