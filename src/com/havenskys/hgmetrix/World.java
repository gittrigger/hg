...
private static String G = "glworld";
	...
	public GLVertex addVertex(float x, float y, float z) {
		...
		GLVertex vertex = new GLVertex(x+mPivotx, y+mPivoty, z+mPivotz, mVertexList.size());
...
/*
		...
		while (iter2.hasNext()) {
			GLVertex vertex = iter2.next();
                        ...
                        if(vertex.x <= -1){vertex.x -= mPivotx;}else{vertex.x += mPivotx;}
                        if(vertex.y <= -1){vertex.y -= mPivoty;}else{vertex.y += mPivoty;}
                        if(vertex.z <= -1){vertex.z -= mPivotz;}else{vertex.z += mPivotz;}
