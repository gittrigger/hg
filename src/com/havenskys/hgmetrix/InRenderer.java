...
mLabelPaint = new Paint();
    ...
    private class RobotTextureLoader implements TextureLoader {
        public void load(GL10 gl) {
            ...
            Bitmap bitmap;
            try {bitmap = BitmapFactory.decodeStream(is);} finally {try {is.close();} catch (IOException e) {}}
...
//        gl.glEnableClientState(GL10.GL_COLOR_ARRAY);
        ...
        gl.glColor4f(0.8f, 0.2f, 0.2f, 0.1f);// traslucent colored background
        gl.glClearColor(0.0f,0.1f,0.2f,0.1f);//red green blue alpha

    gl.glTexEnvx(GL10.GL_TEXTURE_ENV, GL10.GL_TEXTURE_ENV_MODE, GL10.GL_MODULATE);


    float matAmbient[] = new float[] {0.9f, 0.0f, 0.1f, .5f};
    float matDiffuse[] = new float[] {0.0f, 0.5f, 1.0f, .5f};

//    gl.glMaterialfv(GL10.GL_FRONT_AND_BACK, GL10.GL_SHININESS, matAmbient, 0);
    gl.glMaterialfv(GL10.GL_FRONT_AND_BACK, GL10.GL_DIFFUSE, new float[] {.05f,.31f,.51f,0.38f}, 0);
...
/*
int dd = 1; if(mCameraZ > 100){dd = 10;}else if(mCameraZ > 30){dd = 1;}
// orbit

       mAngleY = (float) ((int) mAngleY/dd)*dd; 
       mAngleZ = (float) ((int) mAngleZ/dd)*dd; 
       mAngleX = (float) ((int) mAngleX/dd)*dd; 

       mCameraY = (float) ((int) mCameraY/dd)*dd;       
       mCameraZ = (float) ((int) mCameraZ/dd)*dd;
       mCameraX = (float) ((int) mCameraX/dd)*dd;
//*/
...
//if(mCameraZ < -13 ){mCameraZ = -140;}
        ...
        float d = 0;
        if(mLCameraY == mCameraY ){}else if( mLCameraY > mCameraY ){d = mLCameraY - mCameraY;mLCameraY -= d*0.123f; if(mLCameraY < mCameraY || d < 0.5){mLCameraY = mCameraY;}}else{d = mCameraY - mLCameraY; mLCameraY += d*0.123f; if(mLCameraY > mCameraY || d < 0.5){mLCameraY = mCameraY;}}
        if(mLCameraZ == mCameraZ ){}else if( mLCameraZ > mCameraZ ){d = mLCameraZ - mCameraZ;mLCameraZ -= d*0.123f; if(mLCameraZ < mCameraZ || d < 0.5){mLCameraZ = mCameraZ;}}else{d = mCameraZ - mLCameraZ; mLCameraZ += d*0.123f; if(mLCameraZ < mCameraZ || d < 0.5){mLCameraZ = mCameraZ;}}
        if(mLCameraX == mCameraX ){}else if( mLCameraX > mCameraX ){d = mLCameraX - mCameraX;mLCameraX -= d*0.123f; if(mLCameraX < mCameraX || d < 0.5){mLCameraX = mCameraX;}}else{d = mCameraX - mLCameraX; mLCameraX += d*0.123f; if(mLCameraX < mCameraX || d < 0.5){mLCameraX = mCameraX;}}
        
        	
        //if(mLCameraZ < 5){mLCameraZ = 5f;mCameraZ = 7f;}else if(mLCameraZ > 60){mCameraZ = 60;mCameraZ = 50;}
        //Log.w(G,"GL View("+mLCameraY+"("+mCameraY+"),"+mLCameraX+"("+mCameraX+"),"+mLCameraZ+"("+mCameraZ+"))");
        gl.glTranslatef(mLCameraY, mLCameraX, mLCameraZ);//left-right up-down in-out
        
        gl.glScalef(0.5f, 0.5f, 0.5f);
        //gl.glRotatef(3f,        0, 1, 0);//side
        //gl.glRotatef(16f,  1, 0, 0);//top
        
        if(mLAngleY == mAngleY){}else if( mLAngleY > mAngleY ){d = mLAngleY - mAngleY;mLAngleY -= d*0.123f; if(mLAngleY < mAngleY || d < 0.5){mLAngleY = mAngleY;}}else{d = mAngleY - mLAngleY; mLAngleY += d*0.123f; if(mLAngleY > mAngleY || d < 0.5){mLAngleY = mAngleY;}}
        if(mLAngleZ == mAngleZ){}else if( mLAngleZ > mAngleZ ){d = mLAngleZ - mAngleZ;mLAngleZ -= d*0.123f; if(mLAngleZ < mAngleZ || d < 0.5){mLAngleZ = mAngleZ;}}else{d = mAngleZ - mLAngleZ; mLAngleZ += d*0.123f; if(mLAngleZ < mAngleZ || d < 0.5){mLAngleZ = mAngleZ;}}
        if(mLAngleX == mAngleX){}else if( mLAngleX > mAngleX ){d = mLAngleX - mAngleX;mLAngleX -= d*0.123f; if(mLAngleX < mAngleX || d < 0.5){mLAngleX = mAngleX;}}else{d = mAngleX - mLAngleX; mLAngleX += d*0.123f; if(mLAngleX < mAngleX || d < 0.5){mLAngleX = mAngleX;}}
...
gl.glLightfv(GL10.GL_LIGHT0, GL10.GL_POSITION, new float[]{0,1,20,1.1f,1}, 0);
gl.glLightfv(GL10.GL_LIGHT0, GL10.GL_AMBIENT,  new float[]{0.3f,1,0.7f,.8f}, 0);
gl.glLightf(GL10.GL_LIGHT0, GL10.GL_SPOT_EXPONENT, 60f);
gl.glEnable(GL10.GL_LIGHT1);
gl.glLightfv(GL10.GL_LIGHT1, GL10.GL_POSITION, new float[]{10,3,0,1f,1}, 0);
gl.glLightfv(GL10.GL_LIGHT1, GL10.GL_AMBIENT,  new float[]{1,0,.7f,.8f}, 0);//red,green,blue,alpha
gl.glLightf(GL10.GL_LIGHT1, GL10.GL_SPOT_EXPONENT, 30.0f);
gl.glEnable(GL10.GL_LIGHT2);
gl.glLightfv(GL10.GL_LIGHT2, GL10.GL_POSITION, new float[]{140,3,1f,0.5f,1}, 0);
gl.glLightfv(GL10.GL_LIGHT2, GL10.GL_AMBIENT,  new float[]{.5f,.9f,.8f,.8f}, 0);//red,green,blue,alpha
gl.glLightf(GL10.GL_LIGHT2, GL10.GL_SPOT_EXPONENT, 30.0f);
gl.glEnable(GL10.GL_LIGHT3);
gl.glLightfv(GL10.GL_LIGHT3, GL10.GL_POSITION, new float[]{140,0,0f,1}, 0);
gl.glLightfv(GL10.GL_LIGHT3, GL10.GL_AMBIENT,  new float[]{.8f,.8f,30.9f,0.8f}, 0);//red,green,blue,alpha
...
int ghy = 16;//(int) (mLCameraZ - mWorlds[2].mPivotx)*2;
//if(ghy *-1 > 0){ghy *= -1;}if(ghy < 12){ghy = 12;} if(ghy > 18){ghy = 18;}
mLabelPaint.setTextSize((float)ghy);
mLabelPaint.setARGB(0xff, 0x0F, 0x0f, 0x3f);
        mLabelCo = mLabels.add(gl, "Good Health", mLabelPaint);
        //mLabelCo = mLabels.add(gl, "Good Health "+SystemClock.uptimeMillis(), mLabelPaint);
...
//mCSprite.setValue((int) SystemClock.uptimeMillis() );
float w = mLabels.getWidth(mLabelCo);    
            float x = w - 20;

            mCSprite.draw(gl, x                   , 0, mWidth, mHeight);
...
mProjector.setCurrentView(0, 0, width, height);
        ...
        gl.glLoadIdentity();
        gl.glFrustumf(-ratio, ratio, -1, 1, 1, 560);
...
mProjector.getCurrentProjection(gl);
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        if (mNumericSprite != null) {mNumericSprite.shutdown(gl);} else {mNumericSprite = new NumericSprite();}
        mNumericSprite.initialize(gl, mLabelPaint);

if(mCSprite != null){mCSprite.shutdown(gl);}else{mCSprite = new NumericSprite();}mCSprite.initialize(gl, mLabelPaint);
